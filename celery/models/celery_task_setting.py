...
class CeleryTaskSetting(models.Model):
    ...
    stuck_after_seconds = fields.Integer(
        string="Seems Stuck after seconds",
        tracking=True,
    ...
    use_first_empty_queue = fields.Boolean(
        string="Use the first queue with N or less pending tasks", default=True
    )
    ...
    transaction_strategy = fields.Selection(
        ...
        - API: Programmatically set in the call_task() method kwargs.
          Also used and applied if no Task Setting record exists.""",
    )
    ...
    def _check_hour_range(self):
        ...
        if (self.schedule_hours_from < 0 or self.schedule_hours_from > 24) or (
            self.schedule_hours_to < 0 or self.schedule_hours_to > 24
        ):
...
class CeleryTaskSettingQueue(models.Model):
    ...
    def _default_sequence(self):
        rec = self.search(
            [("task_setting_id", "=", self.task_setting_id.id)],
            limit=1,
            order="sequence DESC",
        )
    ...
    queue_id = fields.Many2one(
        string="Queue", comodel_name="celery.queue", ondelete="cascade", required=True
    )
    active = fields.Boolean(string="Queue Active", related="queue_id.active")
    sequence = fields.Integer(
        string="Sequence", required=True, default=_default_sequence
    )
    queue_max_pending_tasks = fields.Integer(
        string="Use if less then (pending tasks)", default=0, required=True
    )
