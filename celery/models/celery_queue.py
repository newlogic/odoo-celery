...
from .celery_task import STATE_FAILURE, STATE_PENDING, STATE_RETRY, STATE_SUCCESS
...
class CeleryQueue(models.Model):
    ...
    def _get_task_settings(self):
        for record in self:
            task_setting_ids = (
                self.env["celery.task.setting.queue"].search(
                    [("queue_id", "=", record.id)]
                )
                or []
            )
    ...
    def _compute_stats(self):
        if "compute_queue_stats" in self._context:
            ...
            for record in self:
                ...
                record.queue_tasks_pending = queue_tasks.get(
                    record.name + ":pending", 0
                )
                record.queue_tasks_24h = queue_tasks.get(record.name + ":24h", 0)
                record.queue_tasks_24h_done = queue_tasks.get(
                    record.name + ":24h_done", 0
                )
                record.queue_tasks_24h_failed = queue_tasks.get(
                    record.name + ":24h_failed", 0
                )
                queue_tasks_24h = record.queue_tasks_24h
                if queue_tasks_24h == 0:
                    queue_tasks_24h = 1  # avoid division by zero
                record.queue_tasks_ratio = (
                    float(record.queue_tasks) / float(total_tasks)
                ) * 100.00
                record.queue_percentage = (
                    float(record.queue_tasks_24h_done) / float(queue_tasks_24h)
                ) * 100.00
    ...
    # stat fields
    queue_tasks = fields.Integer(
        string="Total number of tasks in the queue",
        compute="_compute_stats",
        store=False,
        group_operator="avg",
    )
    queue_tasks_pending = fields.Integer(
        string="Pending tasks in the queue",
        compute="_compute_stats",
        store=False,
        group_operator="avg",
    )
    queue_tasks_24h = fields.Integer(
        string="Added in the last 24h",
        compute="_compute_stats",
        store=False,
        group_operator="avg",
    )
    queue_tasks_24h_done = fields.Integer(
        string="Succeeded in the last 24h",
        compute="_compute_stats",
        store=False,
        group_operator="avg",
    )
    queue_tasks_24h_failed = fields.Integer(
        string="Failed in the last 24h",
        compute="_compute_stats",
        store=False,
        group_operator="avg",
    )
    queue_percentage = fields.Float(
        string=" ", compute="_compute_stats", store=False, group_operator="avg"
    )
    queue_tasks_ratio = fields.Float(
        string="Percentage of total tasks",
        compute="_compute_stats",
        store=False,
        group_operator="avg",
    )
