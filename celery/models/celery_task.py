...
from ..odoo import TASK_DEFAULT_QUEUE, call_task
...
RETRY_COUNTDOWN_SETTINGS = [
    ...
    (
        RETRY_COUNTDOWN_MULTIPLY_RETRIES_SECCONDS,
        "Multiply retry countdown: retries * seconds",
    ),
]
...
def _get_celery_user_config():
    ...
    password = (
        os.environ.get("ODOO_CELERY_PASSWORD")
        or config.misc.get("celery", {}).get("password")
        or config.get("celery_password")
    )
    sudo = (
        os.environ.get("ODOO_CELERY_SUDO")
        or config.misc.get("celery", {}).get("sudo")
        or config.get("celery_sudo")
    )
...
class CeleryTask(models.Model):
    ...
    user_id = fields.Many2one(
        "res.users", string="User ID", required=True, readonly=True
    )
    ...
    )  # Don't default here (Celery already does)
    interval_step = fields.Float(
        help="On each consecutive retry this number will be added to the Broker Connection retry \
            delay (float or integer). "
        "Default is 0.2."
    )  # Don't default here (Celery already does)
    countdown = fields.Integer(
        help="ETA by seconds into the future. Also used in the retry."
    )
    retry_countdown_setting = fields.Selection(
        selection="_selection_retry_countdown_settings",
        string="Retry Countdown Setting",
    )
    retry_countdown_add_seconds = fields.Integer(string="Retry Countdown add seconds")
    retry_countdown_multiply_retries_seconds = fields.Integer(
        string="Retry Countdown multiply retries seconds"
    )
    ...
    def check_schedule_needed(self, t_setting):
        ...
        # returns a datetime value (in UTC) for when it can be executed next
        # (current moment does not fit the scheduling criteria)
        def get_next_day_of_week_diff(current_day_of_week, allowed_days):
            next_allowed_day_of_week = list(
                filter(lambda day: day > current_day_of_week, allowed_days)
            )
            if next_allowed_day_of_week:
                ...
                return (
                    7
                    - current_day_of_week
                    + list(filter(lambda day: day < current_day_of_week, allowed_days))[
                        0
                    ]
                )

        def get_next_hour_diff(
            current_hour, hour_from, hour_to, current_day_of_week, allowed_days
        ):
            if current_hour <= hour_from:
                if not allowed_days or list(
                    filter(lambda day: day == current_day_of_week, allowed_days)
                ):
                    return hour_from - current_hour
                else:
                    return (hour_from - current_hour) + (
                        get_next_day_of_week_diff(current_day_of_week, allowed_days)
                        * 24
                    )
            else:
                if not allowed_days:
        ...
        current_day_of_week = (
            datetime.now(tz=tz).weekday() + 1
        )  # adding 1 to avoid comparisons to 0
        current_hour = (
            datetime.now(tz=tz).hour
            + (datetime.now(tz=tz).minute / 60.0)
            + (datetime.now(tz=tz).second / 3600.0)
        ) or 0
        allowed_days = list(
            filter(
                None,
                [
                    1 if t_setting.schedule_mondays else False,
        ...
        weekday_scheduling_set = any(
            [
                t_setting.schedule_mondays,
                t_setting.schedule_tuesdays,
                t_setting.schedule_wednesdays,
                t_setting.schedule_thursdays,
                t_setting.schedule_fridays,
                t_setting.schedule_saturdays,
                t_setting.schedule_sundays,
            ]
        )
        hour_scheduling_set = (
            t_setting.schedule_hours_from + t_setting.schedule_hours_to
        ) > 0  # hourly schedule is set if hours set are different from 0
        out_of_allowed_day_range = current_day_of_week not in allowed_days
        out_of_allowed_hour_range = (
            current_hour < t_setting.schedule_hours_from
            or current_hour > t_setting.schedule_hours_to
        )
        if weekday_scheduling_set and out_of_allowed_day_range:
            hour_diff = get_next_hour_diff(
                0,
                t_setting.schedule_hours_from,
                t_setting.schedule_hours_to,
                current_day_of_week,
                allowed_days,
            )
            ...
            scheduled_date = (
                t
                - timedelta(seconds=t.second + (t.minute * 60) + (t.hour * 3600))
                + timedelta(hours=hour_diff)
            ).astimezone(pytz.utc)
        elif (
            weekday_scheduling_set
            and not out_of_allowed_day_range
            and hour_scheduling_set
            and out_of_allowed_hour_range
        ):
            hour_diff = get_next_hour_diff(
                current_hour,
                t_setting.schedule_hours_from,
            ...
            scheduled_date = (
                datetime.now(tz=tz) + timedelta(hours=hour_diff)
            ).astimezone(pytz.utc)
        elif (
            not weekday_scheduling_set
            and hour_scheduling_set
            and out_of_allowed_hour_range
        ):
            hour_diff = get_next_hour_diff(
    ...
    def call_task(self, model, method, **kwargs):
        ...
        # queue selection
        default_queue = (
            kwargs.get("celery", False)
            and kwargs.get("celery").get("queue", "")
            or "celery"
        )
        task_queue = False
        task_setting_domain = [
        ...
        task_setting = (
            self.env["celery.task.setting"].sudo().search(task_setting_domain, limit=1)
        )
        if task_setting:
            ...
            if task_setting.schedule:
                ...
                if scheduled_date:
                    vals.update(
                        {"state": STATE_SCHEDULED, "scheduled_date": scheduled_date}
                    )

            if task_setting.task_queue_ids:
                if task_setting.use_first_empty_queue:
                    for q in task_setting.task_queue_ids.sorted(
                        ...
                        if q.queue_id.active:
                            if (
                                self.search_count(
                                    [
                                        ("queue", "=", q.queue_id.name),
                                        ("state", "=", STATE_PENDING),
                                    ]
                                )
                                <= q.queue_max_pending_tasks
                            ):
                ...
                if not task_queue:
                    ...
                    active_queues = task_setting.task_queue_ids.filtered(
                        lambda q: q.queue_id.active
                    )
                    if active_queues:
        ...
        def apply_call_task():
            ...
            if call_task:
                with api.Environment.manage(), db_registry.cursor() as cr:
                    ...
                    if (
                        not scheduled_date
                    ):  # if the task is not scheduled for a later time
                        Task._celery_call_task(
                            user_id, task_uuid, model, method, kwargs
                        )
    ...
    def _transaction_strategies(self):
        transaction_strategies = (
            self.env["celery.task.setting"]._fields["transaction_strategy"].selection
        )
    ...
    def _transaction_strategy(self, task_setting, kwargs):
        ...
        if (
            task_setting
            and task_setting.transaction_strategy == "api"
    ...
    def rpc_run_task(self, task_uuid, model, method, *args, **kwargs):
        ...
        task_ref = (
            kwargs.get("celery_task_vals", False)
            and kwargs.get("celery_task_vals").get("ref", "")
            or ""
        )
        logger.info(
            "CELERY rpc_run_task uuid:{uuid} - model: {model} - method: {method} - ref: {ref} - queue: {queue}".format(
                uuid=task_uuid,
                model=model,
                method=method,
                ref=task_ref,
        ...
        task = self.search(
            [
                ("uuid", "=", task_uuid),
                ("state", "in", [STATE_PENDING, STATE_RETRY, STATE_SCHEDULED]),
            ],
            limit=1,
        )
        ...
        with registry(self._cr.dbname).cursor() as cr:
            ...
            try:
                if bool(sudo) and sudo:
                    res = getattr(model_obj.with_env(env).sudo(), method)(
                        task_uuid, **kwargs
                    )
                else:
                    res = getattr(model_obj.with_env(env), method)(task_uuid, **kwargs)

                if not res and not bool(res):
                    msg = (
                        "No result/return value for Task UUID: %s. Ensure the task-method returns a value."
                        % task_uuid
                    )
                    logger.error(msg)
                ...
                if task.retry:
                    state = STATE_RETRY
                    logger.warning(
                        "Retry... exception (see task form) from rpc_run_task {uuid} (model: {model} - \
                            method: {method} - ref: {ref} - queue: {queue}): {exc}.".format(
                            uuid=task_uuid,
                            model=model,
                            method=method,
                            ref=task_ref,
                    ...
                    state = STATE_FAILURE
                    logger.warning(
                        "Failure... exception (see task form) from rpc_run_task {uuid} (model: {model} - \
                            method: {method} - ref: {ref} - queue: {queue}): {exc}.".format(
                            uuid=task_uuid,
                            model=model,
                            method=method,
                ...
                logger.debug(
                    "Exception rpc_run_task: {exc_info}".format(
                        # uuid=task_uuid,
                        exc_info=exc_info
                    )
                )
        ...
        for task in self:
            if task.stuck or task.state in states_to_requeue:
                ...
                try:
                    ...
                    self._celery_call_task(
                        task.user_id.id, task.uuid, task.model, task.method, _kwargs
                    )
                except CeleryCallTaskException as e:
    ...
    def action_stuck(self):
        ...
        for task in self:
            if task.state in STATES_TO_STUCK:
                task.write({"stuck": True, "state_date": fields.Datetime.now()})
        return True
