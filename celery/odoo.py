...
from celery.exceptions import MaxRetriesExceededError, Retry, TaskError
...
app = Celery(
    ...
    worker_prefetch_multiplier=os.environ.get(
        "ODOO_CELERY_WORKER_PREFETCH_MULTIPLIER", False
    ),
)
...
def call_task(self, url, db, user_id, task_uuid, model, method, **kwargs):
    ...
    logger.info(
        "{model} {method} - celery.task uuid: {uuid}".format(
            model=model, method=method, uuid=task_uuid
        )
    )
    logger.info(
        "{model} {method} - kwargs: {kwargs}".format(
    ...
    try:
        ...
        if code == TASK_NOT_FOUND:
            ...
            if retry and retry_countdown_setting:
                if retry_countdown_setting == RETRY_COUNTDOWN_ADD_SECONDS:
                    ...
                    retry_countdown_setting == RETRY_COUNTDOWN_MULTIPLY_RETRIES_SECCONDS
                    and retry_countdown_multiply_retries_seconds > 0
                ):
                    countdown = (
                        self.request.retries * retry_countdown_multiply_retries_seconds
                    )
            ...
            if retry:
                ...
                logger.info(
                    "{task_name} retry params: {params}".format(
                ...
                msg = (
                    "Exit task... Failure in Odoo {db} (task: {uuid}, model: {model}, method: {method})\n"
        ...
        if isinstance(e, MaxRetriesExceededError):
            ...
            msg = "[TODO] Failure (caught) MaxRetriesExceededError: db: {db}, \
                task: {uuid}, model: {model}, method: {method}.".format(
                db=db, uuid=task_uuid, model=model, method=method
            )
            logger.error(msg)
